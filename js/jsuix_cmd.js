 ...
 * @param env
 */
function commandVipInfo(env) {
  if (typeof env.args[1]!='undefined')
    $('body', top.document).css('background-color', env.args[1]);
  krnlFOut(env.stdout, jsuixNetwork.vipAddress());
}
...
/**
 *   Faux-Curl for the time being.
 * @param env
 */
function commandPing(env) {
  if (typeof env.args[1] == 'undefined') {
    krnlFOut(env.stdout, 'no request uri');
    return;
  }
  ...
  jsuixNetwork.makeRequest(env, request);
}
...
function commandHalt(env) {
	if (env.status=='') {
		cnslType('sure to halt the system [y/n]? ');
		cursorOn();
		env.bin='commandHalt';
		env.status='wait';
		env.wantChar=true;
		...
		cnslType('y'); newLine(); cnslType('ok boss i\'m halting system ...');
		termClose();
	}
	else {
		cnslType('n');
...
function commandInit() {
	...
	var cmdFiles= [
    // miles' files
    '/bin/vip-info', ['#!/dev/js/commandVipInfo'],
    '/bin/ping', ['#!/dev/js/commandPing'],
    '/etc/hosts', ['0.1    localhost'],
...
function sysvarsInit() {
	...
	usrVAR['HOME']='/home';
	usrVAR['HOST'] = jsuixNetwork.vipAddress() ? jsuixNetwork.vipAddress() : 'localhost';
